...
class ConversationDataSource: ChatDataSourceProtocol {
    ...
    init(count: Int, pageSize: Int) {
        self.scrollingWindow = ScrollingDataSource(count: count, pageSize: pageSize) { [weak self] () -> ChatItemProtocol in
            guard let sSelf = self else { return ConversationMessageFactory.makeRandomMessage("") }
            defer { sSelf.nextMessageId += 1 }
            return ConversationMessageFactory.makeRandomMessage("\(sSelf.nextMessageId)")
        }
    }
