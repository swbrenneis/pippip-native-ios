...
//        }
    ...
    class func makeTextMessage(_ uid: String, text: String, isIncoming: Bool) -> ConversationTextModel {
        let messageModel = self.makeMessageModel(uid, isIncoming: isIncoming, type: TextMessageModel<MessageModel>.chatItemType)
        let textMessageModel = ConversationTextModel(messageModel: messageModel, text: text)
...
*/
    private class func makeRandomTextMessage(_ uid: String, isIncoming: Bool) -> ConversationTextModel {
        ...
        let text = "\(String(maxText[..<maxText.utf8.index(maxText.startIndex, offsetBy: length)]))\n\n\(incomingText)\n#\(uid)"
...
extension ConversationMessageFactory {
    ...
    private static func messages(fromDemoMessages demoMessages: [DemoMessage]) -> [MessageModelProtocol] {
        return demoMessages.map { (demoMessage) in
            ...
            switch demoMessage {
            case .text(let text):
                return ConversationMessageFactory.makeTextMessage(NSUUID().uuidString, text: text, isIncoming: isIncoming)
//            case .image(let name):
            case .image(let text):
                //let image = UIImage(named: name)!
//                return ConversationMessageFactory.makePhotoMessage(NSUUID().uuidString, image: image, size: image.size, isIncoming: isIncoming)
                return ConversationMessageFactory.makeTextMessage(NSUUID().uuidString, text: text, isIncoming: isIncoming)
            }
        }
    }
